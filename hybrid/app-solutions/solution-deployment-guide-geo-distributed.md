...
### <a name="geo-distributed-app-steps"></a><span data-ttu-id="fc6b0-161">Geografisch verteilte App – Schritte</span><span class="sxs-lookup"><span data-stu-id="fc6b0-161">Geo-distributed app steps</span></span>
...
2. <span data-ttu-id="fc6b0-186">**Checken Sie den Code in Azure Repos ein**, indem Sie Team Explorer verwenden.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-186">**Check in the code to Azure Repos** using Team Explorer.</span></span>
...
7. <span data-ttu-id="fc6b0-213">Geben Sie unter **Agent-Warteschlange** für die gehostete Azure Cloud-Umgebung die Zeichenfolge „Hosted VS2017“ ein.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-213">Enter "Hosted VS2017" under **Agent queue** for Azure cloud hosted environment.</span></span>
...
15. <span data-ttu-id="fc6b0-231">Legen Sie den App Service-Namen als den Namen der Azure Stack Hub-Web-App fest.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-231">Set the Azure Stack Hub web app name as the App service name.</span></span>
...
## <a name="part-2-update-web-app-options"></a><span data-ttu-id="fc6b0-248">Teil 2: Aktualisieren der Web-App-Optionen</span><span class="sxs-lookup"><span data-stu-id="fc6b0-248">Part 2: Update web app options</span></span>
...
#### <a name="access-dns-records-with-domain-provider"></a><span data-ttu-id="fc6b0-271">Zugreifen auf DNS-Einträge mit Domänenanbieter</span><span class="sxs-lookup"><span data-stu-id="fc6b0-271">Access DNS records with domain provider</span></span>
...
#### <a name="test-in-a-browser"></a><span data-ttu-id="fc6b0-321">Testen in einem Browser</span><span class="sxs-lookup"><span data-stu-id="fc6b0-321">Test in a browser</span></span>
...
### <a name="requirements-for-your-ssl-certificate"></a><span data-ttu-id="fc6b0-335">Anforderungen an Ihr SSL-Zertifikat</span><span class="sxs-lookup"><span data-stu-id="fc6b0-335">Requirements for your SSL certificate</span></span>
...
#### <a name="bind-your-ssl-certificate-and-merge-intermediate-certificates"></a><span data-ttu-id="fc6b0-363">Binden Ihres SSL-Zertifikats und Zusammenführen von Zwischenzertifikaten</span><span class="sxs-lookup"><span data-stu-id="fc6b0-363">Bind your SSL certificate and merge intermediate certificates</span></span>
...
<span data-ttu-id="fc6b0-371">Exportieren Sie das zusammengeführte SSL-Zertifikat mit dem privaten Schlüssel, der vom Zertifikat generiert wurde.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-371">Export the merged SSL certificate with the private key generated by the certificate.</span></span>
...
<span data-ttu-id="fc6b0-383">Wenn der Upload des Zertifikats in App Service abgeschlossen ist, wird es auf der Seite **SSL-Einstellungen** angezeigt.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-383">When App Service finishes uploading the certificate, it appears in the **SSL settings** page.</span></span>
...
4. <span data-ttu-id="fc6b0-397">Wählen Sie **Bindung hinzufügen**.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-397">Select **Add Binding**.</span></span>
...
2. <span data-ttu-id="fc6b0-425">Wählen Sie unter **TLS-Version** die TLS-Mindestversion aus.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-425">In **TLS version**, select the minimum TLS version.</span></span>
...
7. <span data-ttu-id="fc6b0-447">Verwenden Sie für den vollqualifizierten Domänennamen (**FQDN**) die externe URL für die Azure Stack Hub-Web-App.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-447">For fully qualified domain name (**FQDN**), use the external URL for the Azure Stack Hub Web App.</span></span>

8. <span data-ttu-id="fc6b0-448">Wählen Sie unter „Geografische Zuordnung“ die Region bzw. den Kontinent der Ressource aus.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-448">Under Geo-mapping, select a region/continent where the resource is located.</span></span> <span data-ttu-id="fc6b0-449">Beispiel: **Europa**.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-449">For example, **Europe.**</span></span>

9. <span data-ttu-id="fc6b0-450">Wählen Sie in der angezeigten Dropdownliste „Land/Region“ das Land aus, das für diesen Endpunkt gilt.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-450">Under the Country/Region drop-down that appears, select the country that applies to this endpoint.</span></span> <span data-ttu-id="fc6b0-451">Beispiel: **Deutschland**.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-451">For example, **Germany**.</span></span>

10. <span data-ttu-id="fc6b0-452">Lassen Sie **Als deaktiviert hinzufügen** deaktiviert.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-452">Keep **Add as disabled** unchecked.</span></span>

11. <span data-ttu-id="fc6b0-453">Klicken Sie auf **OK**.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-453">Select **OK**.</span></span>

12. <span data-ttu-id="fc6b0-454">Hinzufügen des Azure-Endpunkts:</span><span class="sxs-lookup"><span data-stu-id="fc6b0-454">Adding the Azure Endpoint:</span></span>

    1. <span data-ttu-id="fc6b0-455">Wählen Sie für **Typ** die Option **Azure-Endpunkt**.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-455">For **Type**, select **Azure endpoint**.</span></span>

    2. <span data-ttu-id="fc6b0-456">Geben Sie einen **Namen** für den Endpunkt an.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-456">Provide a **Name** for the endpoint.</span></span>

    3. <span data-ttu-id="fc6b0-457">Wählen Sie unter **Zielressourcentyp** die Option **App Service**.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-457">For **Target resource type**, select **App Service**.</span></span>

    4. <span data-ttu-id="fc6b0-458">Wählen Sie unter **Zielressource** die Option **App Service auswählen**, um die Auflistung der Web-Apps desselben Abonnements anzuzeigen.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-458">For **Target resource**, select **Choose an app service** to show the listing of the Web Apps under the same subscription.</span></span> <span data-ttu-id="fc6b0-459">Wählen Sie unter **Ressource** die App Service-Instanz aus, die als erster Endpunkt verwendet werden soll.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-459">In **Resource**, pick the App service used as the first endpoint.</span></span>

13. <span data-ttu-id="fc6b0-460">Wählen Sie unter „Geografische Zuordnung“ die Region bzw. den Kontinent der Ressource aus.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-460">Under Geo-mapping, select a region/continent where the resource is located.</span></span> <span data-ttu-id="fc6b0-461">Beispiel: **Nordamerika/Zentralamerika/Karibik**.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-461">For example, **North America/Central America/Caribbean.**</span></span>

14. <span data-ttu-id="fc6b0-462">Lassen Sie das Feld der Dropdownliste „Land/Region“ leer, um alle obigen regionalen Gruppierungen auszuwählen.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-462">Under the Country/Region drop-down that appears, leave this spot blank to select all of the above regional grouping.</span></span>

15. <span data-ttu-id="fc6b0-463">Lassen Sie **Als deaktiviert hinzufügen** deaktiviert.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-463">Keep **Add as disabled** unchecked.</span></span>

16. <span data-ttu-id="fc6b0-464">Klicken Sie auf **OK**.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-464">Select **OK**.</span></span>

    > [!Note]  
    >  <span data-ttu-id="fc6b0-465">Erstellen Sie mindestens einen Endpunkt mit dem geografischen Bereich „Alle (Welt)“, der als Standardendpunkt für die Ressource dient.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-465">Create at least one endpoint with a geographic scope of All (World) to serve as the default endpoint for the resource.</span></span>

17. <span data-ttu-id="fc6b0-466">Wenn Sie das Hinzufügen beider Endpunkte abgeschlossen haben, werden diese unter **Traffic Manager-Profil** zusammen mit ihrem Überwachungsstatus als **Online** angezeigt.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-466">When the addition of both endpoints is complete, they're displayed in **Traffic Manager profile** along with their monitoring status as **Online**.</span></span>

    ![Endpunktstatus des Traffic Manager-Profils](media/solution-deployment-guide-geo-distributed/image46.png)

#### <a name="global-enterprise-relies-on-azure-geo-distribution-capabilities"></a><span data-ttu-id="fc6b0-468">Globales Unternehmen nutzt Azure-Funktionen für die geografische Verteilung</span><span class="sxs-lookup"><span data-stu-id="fc6b0-468">Global Enterprise relies on Azure geo-distribution capabilities</span></span>

<span data-ttu-id="fc6b0-469">Die Weiterleitung des Datenverkehrs mit Azure Traffic Manager und geografieabhängigen Endpunkten ermöglicht es globalen Unternehmen, regionale Bestimmungen einzuhalten und für die Konformität und den Schutz der Daten zu sorgen, was für den Erfolg von lokalen und Remoteunternehmensstandorten von entscheidender Bedeutung ist.</span><span class="sxs-lookup"><span data-stu-id="fc6b0-469">Directing data traffic via Azure Traffic Manager and geography-specific endpoints enables global enterprises to adhere to regional regulations and keep data compliant and secure, which is crucial to the success of local and remote business locations.</span></span>

## <a name="next-steps"></a><span data-ttu-id="fc6b0-470">Nächste Schritte</span><span class="sxs-lookup"><span data-stu-id="fc6b0-470">Next steps</span></span>

- <span data-ttu-id="fc6b0-471">Weitere Informationen zu Azure-Cloudmustern finden Sie unter [Cloudentwurfsmuster](/azure/architecture/patterns).</span><span class="sxs-lookup"><span data-stu-id="fc6b0-471">To learn more about Azure Cloud Patterns, see [Cloud Design Patterns](/azure/architecture/patterns).</span></span>
